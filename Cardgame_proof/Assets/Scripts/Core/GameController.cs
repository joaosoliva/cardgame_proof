...
namespace CardgameProof.Core
{
    public sealed class GameController : MonoBehaviour
    {
        ...
        private void BeginSetupForPlayer(PlayerId player)
        {
            ...
            boardController?.BuildBoard(sceneRoot.CenterBoardArea, ActiveModeConfig.BoardSize, null);
            if (boardController != null) boardController.OnPlacedCardTapped = OnPlacedCardTapped;
            boardController?.RefreshVisualsForPhase(CurrentPhase);
        ...
        {
            if (sceneRoot == null || sceneRoot.OverlayLayer == null || readyScreenView != null) return;
        ...
        private void HideSetupSensitiveUi()
        {
            ...
            if (sceneRoot != null && sceneRoot.CenterBoardArea != null)
            {
                CanvasGroup cg = sceneRoot.CenterBoardArea.GetComponent<CanvasGroup>();
                if (cg == null) cg = sceneRoot.CenterBoardArea.gameObject.AddComponent<CanvasGroup>();
                cg.alpha = 0.05f;
            }
        }
        ...
        private void StartInvestigationPhase()
        {
            ...
            if (boardController != null)
            {
                boardController.ClearBoard();
                boardController.BuildBoard(sceneRoot.CenterBoardArea, ActiveModeConfig.BoardSize, null);
                foreach (PlacedCardData card in playerBoardStates[PlayerId.PlayerTwo])
                {
                    PlacedCardData hidden = new PlacedCardData { CardId = card.CardId, CardType = card.CardType, Owner = card.Owner, Coordinate = card.Coordinate, IsFaceUp = false };
                    boardController.PlaceCard(hidden);
                }
                boardController.RefreshVisualsForPhase(CurrentPhase);
            }
            Debug.Log("Investigação iniciada com privacidade preservada.");
        }

        private void StorePlacedCardForCurrentPlayer(PlacedCardData placed)
        {
            if (!playerBoardStates.ContainsKey(currentSetupPlayer)) playerBoardStates[currentSetupPlayer] = new List<PlacedCardData>();
            RemoveStoredPlacedCardForCurrentPlayer(placed.CardId);
        ...
        private void RemoveStoredPlacedCardForCurrentPlayer(string cardId)
        {
            if (!playerBoardStates.ContainsKey(currentSetupPlayer)) return;
            playerBoardStates[currentSetupPlayer].RemoveAll(c => c.CardId == cardId);
        }
